...
<details open>
  ...
  <div style="padding-left: 20px; margin-top: 10px;">
    <ul>
      ...
      <li><b>Умная генерация конфига:</b> Читает UCI и собирает минималистичный <code>/var/etc/telemt.toml</code>. Фича выключена — в TOML её не будет.</li>
      ...
      <li><b>Суровый парсинг версии:</b> Скрипт достает версию бинарника для отображения в UI, используя команду <code>grep -a -m1 -i 'MTProxy v' /usr/bin/telemt</code>. <b>Почему мы не делаем красиво через <code>telemt --version</code>?</b> Потому что запуск бинарника в MIPS-среде только ради вывода версии может повиснуть, выдать ошибку отсутствующей либы или просто сожрать процессорное время. Выдирание строки прямо из кишок скомпилированного файла через <code>grep</code> работает безотказно и моментально, как удар кувалдой.</li>
      <li><b>Обход глухого NAT (STUN Fallback):</b> Если провайдер режет UDP/STUN, скрипт замечает выключенный STUN в настройках и жестко скармливает Announce IP листенеру. В итоге Middle-End Proxy поднимается даже за CGNAT.</li>
